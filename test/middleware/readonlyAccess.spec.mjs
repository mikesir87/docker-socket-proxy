...
describe("ReadonlyAccess", () => {
  ...
  describe("applies", () => {
    ...
    it("does apply to PUT requests", () => {
      expect(middleware.applies("PUT", new URL("http://localhost/images/create"))).toBe(true);
    });

    it("does apply to DELETE requests", () => {
      expect(middleware.applies("DELETE", new URL("http://localhost/containers/create"))).toBe(true);
    });
  ...
  describe("run", () => {
    it("blocks access on a PUT request", () => {
      expect(() => middleware.run({}, {}, {})).toThrow("Read-only access is enabled");
    });
  });
