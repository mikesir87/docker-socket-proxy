...
describe("RegistryBlocker", () => {
  ...
  it("requires a registries config", () => {
    expect(() => new RegistryBlockerMiddleware({})).toThrow("Missing 'registries' in config");
  });
  ...
  describe("run", () => {
    it("blocks access to a forbidden registry", () => {
      ...
      const url = new URL("http://localhost/images/create?fromImage=quay.io/foo/bar&tag=latest");
      const body = {};

      expect(() => middleware.run(requestOptions, url, body)).toThrow("Access to registry quay.io is forbidden");
    });
    ...
    it("does not block access to an allowed registry (docker.io default)", () => {
      const requestOptions = {};
      const url = new URL("http://localhost/images/create?fromImage=foo/bar&tag=latest");
      const body = {};
