...
describe("RewriteVolumePathMiddleware", () => {
  ...
  describe("constructor", () => {
    it("requires a from setting", () => {
      expect(() => new RewriteVolumePathMiddleware({ to : "test" })).toThrow("Missing 'from' in config");
    });

    it("requires a to setting", () => {
      expect(() => new RewriteVolumePathMiddleware({ from : "test" })).toThrow("Missing 'to' in config");
    });
  });
  ...
  describe("applies", () => {
    ...
    it("does not apply to GET /containers/create", () => {
      expect(middleware.applies("GET", new URL("http://localhost/containers/create"))).toBe(false);
    });

    it("does not apply to POST /images/create", () => {
      expect(middleware.applies("POST", new URL("http://localhost/images/create"))).toBe(false);
    });
  });
  ...
  it("remaps when additional path is specified", () => {
    const body = {
      HostConfig: {
        Binds: [
          `${FROM}/additional/path:/some/other/path`
        ]
      }
    };
    ...
    expect(body.HostConfig.Binds).toEqual([`${TO}/additional/path:/some/other/path`]);
  });
  ...
  it("correctly handles trying to break out", () => {
    ...
    expect(body.HostConfig.Binds).toEqual([`${FROM}/../../path:/some/other/path`]);
  });
  ...
  it("correctly handles data in front of the from", () => {
    ...
    expect(body.HostConfig.Binds).toEqual([`/test/${FROM}/../../path:/some/other/path`]);
  });
});
